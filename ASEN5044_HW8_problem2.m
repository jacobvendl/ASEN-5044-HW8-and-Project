...
%now make the unperturbed measurement model for comparison later
...
for i=1:12
    plot(T,rhoDot_per(i,:),'-','LineWidth',1.5)
    plot(T,rhoDot_per(i,:) + dy_lin(3*i-1,:),'k--','LineWidth',1.5)
end
xlabel('Time [s]'); ylabel('\rhoDot^i [km/s]')

subplot(3,1,3); hold on; grid on; grid minor;
for i=1:12
    plot(T,phi_per(i,:),'-','LineWidth',1.5)
    plot(T,phi_per(i,:) + dy_lin(3*i,:),'k--','LineWidth',1.5)
end
subplot(3,1,1); hold on; grid on; grid minor;
for i=1:12
    plot(T,rho_per(i,:),'-','LineWidth',1.5)
    plot(T,rho_per(i,:) + dy_lin(3*i-2,:),'k--','LineWidth',1.5)
end
...
legend('Deviated Trajec - Nominal Trajec','Linearized Deviations')
